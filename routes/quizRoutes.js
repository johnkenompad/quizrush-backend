...
router.post('/generate-quiz', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      (Number(mcQuestions) > 0 || Number(tfQuestions) > 0 || Number(essayQuestions) > 0)
    ) {
      console.log('🔄 Using fallback mode');
      for (const type of types) {
        let count = 0;
        if (type === 'mc') count = Number(mcQuestions);
        else if (type === 'tf') count = Number(tfQuestions);
        else if (type === 'essay') count = Number(essayQuestions);
...
router.post('/generate-custom-quiz', express.json(), async (req, res) => {
  ...
  const { title, difficulty, mcqCount = 0, tfCount = 0, essayCount = 0, created_by } = req.body;
  ...
  try {
    ...
    const config = [
      ...
      { type: 'essay', count: Number(essayCount || 0) },
    ];
    ...
    const total     = Number(mcqCount || 0) + Number(tfCount || 0) + Number(essayCount || 0);
...
function reshapeQuestion(q, type, bloomLabel = '', contentArea = '') {
  ...
  if (type === 'mc') {
    ...
    // Find the correct answer and return its index
    const answerText = (q.answer || '').replace(/^[A-D]\.\s*/i, '').trim().toLowerCase();
    const correctIndex = opts.findIndex(
      (o) => o.replace(/^[A-D]\.\s*/i, '').trim().toLowerCase() === answerText
    );
    ...
    return {
      ...
      question     : (q.question || '').trim(),
      options      : opts,
      correct_answer: correctIndex >= 0 ? correctIndex : 0, // Return index instead of text
      bloom_level  : bloomLabel,
      content_area : contentArea || undefined,
    };
  }
  ...
  if (type === 'essay') {
    return {
      ...
      content_area : contentArea || undefined,
      rubric       : q.rubric || {
        relevance: 'Content directly addresses the question',
        clarity  : 'Ideas are clearly expressed and well-organized',
        depth    : 'Shows understanding and critical thinking',
...
[
  ...
  if (type === 'essay') {
    return `
Return only valid JSON.
Generate ${num} essay questions on the topic "${text}" (difficulty: "${difficulty}").
Each question should require critical thinking and detailed responses.
JSON format:
[
  {
    "question": "...",
...
[
  ...
  if (type === 'essay') {
    return `
Return only valid JSON.
Generate ${num} essay questions on "${text}" at Bloom level "${bloomLabel}" (difficulty: "${difficulty}").
Each question should require critical thinking and detailed responses.
JSON format:
[
  {
...
[
  ...
  if (type === 'essay') {
    return `
Return only valid JSON.
Generate ${num} essay questions about "${topic}" at Bloom level "${bloomLabel}" (difficulty: "${difficulty}").
Each question should require critical thinking and detailed responses.
JSON format:
[
  {
